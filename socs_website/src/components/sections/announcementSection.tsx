...
const poppins4 = Poppins({ weight: "500", subsets: ["latin"] });
...
const AnnouncementsCardComponent = ({ title, description, imageSrc }: Announcement) => {
    return (
        <div className="relative w-full h-full group rounded-[17px] overflow-hidden">
            ...
            <div className="absolute bottom-0 w-full bg-black  text-white p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <h3 className={`${poppins5.className} text-[40px] font-semibold`}>{title}</h3>
                <p className={`${poppins4.className} text-sm`}>{description}</p>
            </div>
        </div>
    );
};
...
const AnnouncementsPage = () => {
    ...
    return (
        <section className="text-center py-16 bg-black px-4 sm:px-8 md:px-16 lg:px-24 xl:px-32">
            <h2 className={`${poppins5.className} text-5xl py-7 font-semibold mb-4 text-white`}>ANNOUNCEMENT</h2>
